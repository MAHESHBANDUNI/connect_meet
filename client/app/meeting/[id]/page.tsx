...
export default function MeetingPage() {
  ...
  const handleParticipantRoleChange = async(targetUserId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/meetings/${meetingDetails?.meetingId}/role-change`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session?.user?.token}`
      ...
      if (!response.ok) throw new Error('Failed to change participant role');
      successToast('Participant role changed successfully');
    } catch (err) {
      console.error('Error changing participant role:', err);
      errorToast('Failed to change participant role');
    }
  }
        ...
        {isInviteModalOpen && (
          <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl max-w-xl w-full overflow-hidden">
              {/* Modal Header */}
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-2 sm:p-4 text-white">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 sm:gap-3">
                    <Mail className="w-4 h-4 sm:w-6 sm:h-6" />
                    <div>
                      <h2 className="text-md sm:text-xl font-semibold sm:font-bold">Invite Participants</h2>
              ...
              <div className="p-2 sm:p-6 mt-1.5 sm:mt-0">
                <div className="mb-3 sm:mb-6">
                  ...
                  <div className="flex gap-2 text-xs sm:text-sm">
                    <input
                      ...
                      className="flex-1 px-2 sm:px-4 py-1.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                      value={newEmail}
                    ...
                    <button
                      ...
                      className="bg-blue-600 text-white px-2 sm:px-4 py-1.5 sm:py-3 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center gap-1 sm:gap-2 min-w-[80px] justify-center"
                    >
                    ...
                    {emailList.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {emailList.map((email, index) => (
                          <div
                            key={index}
                            className="flex items-center gap-1 justify-between bg-white p-1 sm:p-3 rounded-lg border border-gray-100 hover:bg-blue-50 transition-colors group"
                          >
                            ...
                            >
                              <X className="w-2.5 sm:w-4 h-2.5 sm:h-4" />
                            </button>
                          </div>
                        ))}
                      </div>
                ...
                {emailList.length > 0 && (
                  <div className="mb-3 sm:mb-6 p-1.5 sm:p-4 bg-blue-50 rounded-lg border border-blue-100">
                ...
                <div className="flex gap-3 pt-1.5 sm:pt-4 border-t border-gray-200 text-sm sm:text-md">
                  <button
                    onClick={() => {
                      setIsInviteModalOpen(false);
                      setEmailList([]);
                    ...
                    disabled={isSending}
                    className="flex-1 px-2 sm:px-4 py-1.5 sm:py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 disabled:opacity-50 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 bg-blue-600 text-white px-2 sm:px-4 py-1.5 sm:py-3 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
                    {isSending ? (
                      <>
